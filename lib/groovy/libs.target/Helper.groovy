...
class Helper {
	...
	def Helper(groovyShell, scheduler){
		Helper.sched = scheduler
		Helper.shell = groovyShell
	}
	...
	def parseJobFile(jobFile){
		...
		def osInfo = null
		//Get OSINFO
		osInfo = binding.hasVariable(ResourceConstants.OSINFO)?binding.getVariable(ResourceConstants.OSINFO):null
		//Set process runner for os monitoring job
		def processRunnerClz  = Helper.shell.getClassLoader().loadClass('ProcessRunner')
		def procRunner = processRunnerClz.newInstance(osInfo)
		binding.setVariable("procRunner", procRunner)
		
		if(binding.hasVariable(ResourceConstants.DEF_COMMAND )){
			dataMap[ResourceConstants.DEF_COMMAND] = binding[ResourceConstants.DEF_COMMAND]
		}
		if(binding.hasVariable(ResourceConstants.DEF_FORMAT)){
			dataMap[ResourceConstants.DEF_FORMAT] = binding[ResourceConstants.DEF_FORMAT]
		}
		if(binding.hasVariable(ResourceConstants.DEF_FINALLY)){
			dataMap[ResourceConstants.DEF_FINALLY] = binding[ResourceConstants.DEF_FINALLY]
		}
	...
	def loadParams(binding, param_file){
		def param = loadData(param_file)
		binding.setVariable('parameters', param)
		return binding
	}	
	...
	def getBinding(jobFile){
		...
		def o = loader.parseClass(jobFile).newInstance()
                ...
                def param_file = new File("../var/conf/default.params")
		def parameters = shell.evaluate(param_file)
